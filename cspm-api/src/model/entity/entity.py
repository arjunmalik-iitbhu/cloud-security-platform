...
class Credential(SQLModel, table=True):
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ...
            ondelete="CASCADE",
            onupdate="CASCADE",
            name="credential_foreign_key_cloud_id",
        ),
...
class Resource(SQLModel, table=True):
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ondelete="CASCADE",
            onupdate="CASCADE",
            name="resource_foreign_key_cloud_id",
        ),
    ...
    details: dict = Field(
        sa_column=Column(
            "details", JSON, nullable=False, server_default=text("'{}'::json")
        )
    )
    cloud_id: int = Field(sa_column=Column("cloud_id", BigInteger, nullable=False))
    created_at: datetime.datetime = Field(
    ...
    external_resource_id: Optional[str] = Field(
        default=None, sa_column=Column("external_resource_id", String(2000))
    )
...
class Analysis(SQLModel, table=True):
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ondelete="CASCADE",
            onupdate="CASCADE",
            name="analysis_foreign_key_credential_id",
        ),
        ForeignKeyConstraint(
            ["resource_id"],
            ["resource.id"],
            ondelete="CASCADE",
            onupdate="CASCADE",
            name="analysis_foreign_key_resource_id",
        ),
    ...
    credential_id: int = Field(
        sa_column=Column("credential_id", BigInteger, nullable=False)
    )
    resource_id: int = Field(
        sa_column=Column("resource_id", BigInteger, nullable=False)
    )
    created_at: datetime.datetime = Field(
        sa_column=Column(
            "created_at",
    ...
    current_resource_status: Optional[str] = Field(
        default=None,
        sa_column=Column(
            "current_resource_status",
            Enum("running", "stopped", name="resource_status"),
        ),
    )
    current_resource_risk: Optional[str] = Field(
        default=None,
        sa_column=Column(
            "current_resource_risk", Enum("low", "high", name="resource_risk")
        ),
    )
